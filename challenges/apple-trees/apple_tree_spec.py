...
class ValidateAppleTreeClass(unittest.TestCase):
    ...
    def test_number_of_apples(self):
        """When tree is 7 years old, should have at least 200 apples"""
        for _ in range(0, 100):  # checks 100 times
            apple_tree = AppleTree()
            for _ in itertools.repeat(None, 7):
                apple_tree.age_tree()
            self.assertGreaterEqual(len(apple_tree.apples), 200)
