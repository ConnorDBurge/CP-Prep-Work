...
class AssociationTestCase(TestCase):
    def setUp(self):
        ...
        self.order_food_item_1 = OrderFoodItem.objects.create(
            order=self.order, food_item=self.food_item_1)
        self.order_food_item_2 = OrderFoodItem.objects.create(
            order=self.order, food_item=self.food_item_2)
        self.order_food_item_3 = OrderFoodItem.objects.create(
            order=self.order, food_item=self.food_item_3)
        self.order_food_item_4 = OrderFoodItem.objects.create(
            order=self.order, food_item=self.food_item_4)
    ...
    def test_05_orders_food_items(self):
        ...
        self.assertEqual(list(self.order.food_items.all()), [
                         self.food_item_1, self.food_item_2, self.food_item_3, self.food_item_4])
    ...
    def test_07_users_food_items(self):
        ...
        all_food_items = list(list((order.food_items.all())
                              for order in list(self.user.orders.all()))[0])
        self.assertEqual(all_food_items, [
                         self.food_item_1, self.food_item_2, self.food_item_3, self.food_item_4])
