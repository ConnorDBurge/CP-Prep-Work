...
def validate_choices(value):
    checks = ['front crawl', 'butterfly', 'breast', 'back', 'freestyle']
...
def validate_distance(value):
    if value < 50:
        raise ValidationError(
            f"Ensure this value is greater than or equal to 50.")


def validate_record_date(date):
...
class SwimRecord(models.Model):
    ...
    stroke = models.CharField(
        max_length=255, null=False, validators=[validate_choices])
    distance = models.IntegerField(null=False, validators=[validate_distance])
    record_date = models.DateTimeField(
        null=False, validators=[validate_record_date])
